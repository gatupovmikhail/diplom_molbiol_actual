#11233806
#er 0
# gnomad_filtr.py && concan_gnomad.py && gnomad_likv.py && num_of_pol_gnom.py && num_of_pol2gnom.py &&changing_genom40..py || comparing_gens.py
# неработоспособная программа
from sys import exit
from my_library import writting_file, checkfile, zagprint, few_columns

if __name__=="__main__":
    #name_file = 'gnomad_polim_itog.vcf'
    name_file = 'num_pol_new_2.txt'
    name_genome_file='genom.fa'
    #name_file_out = 'genom_comp_gnom.txt'
    name_file_out = 'genom_comp_db.txt'
    file_error=open('gnomadd_errors.txt','w')
    full_error=open('gnomadd_full_errors.txt','w')
    ...
    #name_file_pol = 'gnomad_polim.vcf'
    ...
    with open(name_file) as mfile:
        ...
        for st in mfile:
            ...
            for po in polim:
                ...
                if not(alt == refg) and not(ref == refg) and (loc=='e'): ############beg
                    if al_f=='ref':
                        rep=ref
                    if al_f=='alt':
                        rep=alt
    ...
    # if name==_main_
    print(num_pol)
    print('alt_pol {}'.format(alt_pol))
    print('ref_pol {}'.format(ref_pol))
    print('er {}'.format(er))
